...
Weekly equivalised **mean** income amount in each survey (includes all households):
```{r mean_components}

tidydata %>%
  group_by(survey, type) %>%
  summarise(mean = comma(wtd.mean(amount, weights = hhwgt), prefix = "£")) %>%
  spread(survey, mean) %>%
  mutate(type = factor(type, levels = inctypes)) %>%
