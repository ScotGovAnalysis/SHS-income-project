...
tidyhbai <- hbai1819 %>%
  ...
  mutate(simd = NA,
         ...
         hcost = ehcost,
         tenure = case_when(PTENTYP2 %in% c(1,2,3) ~ PTENTYP2,
                            PTENTYP2 == 4 ~ 3,
                            PTENTYP2 == 5 ~ 4,
                            TRUE ~ 5),
  ...
  gather(key = type, value = amount,
         -ID, -SERNUM, -hhwgt, -ppwgt, -chwgt, -wawgt, -pnwgt, -pp, -ch, -wa,
         -pn, -council, -urbrur, -hhtype, -HIHemp, -equ, -tenure, -hcost,
         -simd) %>%
  mutate(survey = "HBAI") %>%
  remove_labels()
...
# add empty SHS variables
tidyhbai$finman <- NA
tidyhbai$md <- NA
tidyhbaibens$finman <- NA

saveRDS(tidyhbai, "output/tidyhbai.rds")
saveRDS(tidyhbaibens, "output/tidyhbaibens.rds")
