...
tidyshs19bens <- filter(tidyshs19, type == "ben")

## Aggregate benefit amounts from all adults

bentypesSHS19 <- tidyshs19bens %>%
  select(ID, bencodes, bencodesoa)

for (i in seq(1, 40)){
  bentypesSHS19[, i+1] = bentypesSHS19[, i+1] + bentypesSHS19[, i+41] + bentypesSHS19[, i+81] + bentypesSHS19[, i+121] }

bentypesSHS19 <- bentypesSHS19 %>%
  select(ID, bencodes)

## Transpose to long format and equivalise ----

tidyshs19bens <- tidyshs19bens %>%
...
tidyshs19bens$type <- decode(tidyshs19bens$type,
                           search = bencodes,
