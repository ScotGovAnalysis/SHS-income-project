...
tidydata %>%
  ...
  summarise(tot = sum(amount*hhwgt*equ),
            ...
            median = wtd.quantile(amount, probs = 0.5, weights = hhwgt),
...
tidybens %>%
  ...
  group_by(survey, hhtype) %>%
  summarise(tot = sum(amount*hhwgt),
            median = wtd.quantile(amount, probs = 0.5, weights = hhwgt),
...
- strange: single adult benefit income should be lower in SHS compared to HBAI (see above) but doesn't look like it here
