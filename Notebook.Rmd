...
for (i in seq(1, 40)){
   bentypesSHS[, i+1] = bentypesSHS[, i+1] + bentypesSHS[, i+41] + bentypesSHS[, i+42] + bentypesSHS[, i+43] }

bentypesSHS <- bentypesSHS %>%
  select(ID, bencodes)

tidyshsbens2 <- tidyshsbens %>%
  select(-bencodesoa, -bencodes) %>%
  left_join(bentypesSHS, by = "ID") %>%
  
  select(-type, -amount, -pp, -ch, -wa, -pn) %>%
  gather(type, amount, -ID, -weight, -chwgt, -wawgt, -pnwgt, -council, -urbrur,
...
tidyshsbens2$type <- decode(tidyshsbens2$type,
                           search = bencodes,
                           replace = bennames)
