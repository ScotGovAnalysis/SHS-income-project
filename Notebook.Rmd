...
tidyshs <- hhold18 %>%
  ...
  mutate(equ = 0.67 + (pp-u14-1)*0.33 + u14*0.2,
         ...
         privben = MSCINC02*7/(365*equ) + MSCINC06*7/(365*equ),
  ...
  select(weight, chwgt, wawgt, pnwgt, 
         total, earn, ben, privben, occ, inv, oth, 
...
tidyhbai <- hbai1819 %>%
  ...
  mutate(earn = enternhh/eqobhchh,
         ben = ebeninhh/eqobhchh,
         privben = epribnhh/eqobhchh,
         occ = hntocchh/eqobhchh,
         inv = hntinvhh/eqobhchh,
         oth =  emiscihh/eqobhchh,
         total = earn + ben + privben + occ + inv + oth,
         pp = ADULTH + DEPCHLDH,
         ch = DEPCHLDH,
                                       ...
                                       ifelse(ch >= 1 & wa == 1 & pn == 0, 4, 
                                              ifelse(ch >= 1 & wa == 2 & pn == 0, 5, 
                                                     ifelse(pp == 1 & pn == 1, 6, 
                                                            ifelse(pp == 2 & pn == 2, 7, 
                                                                   ifelse(pp == 2 & wa == 1 & pn == 1, 8, 9))))))))) %>%
  select(weight, chwgt, wawgt, pnwgt, hhtype, HIHemp, pp, ch, wa, pn, 
         total, earn, ben, privben, occ, inv, oth, SERNUM) %>%
  left_join(tidyhousehol, by = "SERNUM") %>%
  gather(key = type, value = amount, -SERNUM, -weight, -chwgt, -wawgt, -pnwgt, -pp, -ch, -wa, -pn, -council, -urbrur, -hhtype, - HIHemp) %>%
  mutate(survey = "HBAI") %>%
  select(-SERNUM) %>%
  remove_labels() 
...
SHS total income is `r components$reldiff[1]` lower than HBAI total income. The differences come mainly from lower benefit and much lower investment income in the SHS, and then also from lower occupational pensions. Little would be gained from further investigating the other income components as these contribute only little to the overall differences in income.

Do investment incomes matter for poverty (= low income) measurement though?

# Employment income

Aggrgeated earnings income matches, but also check medians, as this is the most important income type.

# Benefit income

- median benefit income
- number of benefit (any) recipients
