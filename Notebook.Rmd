...
We compared SHS 2018 income data (the new, broader definition) with HBAI income data. For the HBAI data, we used income before top-income adjustment, and ignored child income (except for benefits) {>> check this is correct >>} and any income deductions {>> give examples of deductions >>}.

{>> Deductions: do they have a bearing on self-employed earnings?? <<}
...
Note that all income discussed here is net income after benefits and tax, and not taking housing costs into account (BHC income).

## Total income overall

The income **distributions** in the SHS and the HBAI look largely similar, with the SHS income distribution being slightly wider, and the SHS **median** slightly lower. Near the (official) relative poverty threshold, the SHS cumulative income curve is higher, meaning more households sit below that threshold in the SHS compared to the HBAI. We will later see that this means that more people are considered to be in relative poverty when using the SHS dataset compared to the HBAI dataset.

The nine income decile points (the thresholds in each dataset that split the populations in ten equal-sized groups) are at similar levels for high incomes and diverge towards lower incomes, with SHS incomes being lower. This shows that more households in the SHS have lower incomes compared to the HBAI, whereas the high income end of the distribution matches better. 

The diverging distributions near the poverty line suggests that it would be advisable to calculate poverty thresholds for each survey separately, based on each survey's data.
...
## Income components conclusions

Total income aggregated to the  whole of Scotland is largely made up of earnings, followed by benefits (which include the state pension) and occupational pensions. Overall, SHS total income is `r incdiff` lower than HBAI total income. The differences come mainly from lower benefit and much lower investment income in the SHS, and then also from lower occupational pensions. Income from earnings are slightly higher in the SHS. Other income components are relatively small and therefore contribute little to the overall differences in income. {>> Do investment incomes matter for poverty (= low income) measurement though? <<}
